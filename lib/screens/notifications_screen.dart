...
class _NotificationsScreenState extends State<NotificationsScreen> {
  ...
  Future<void> _open(AppNotification notification, String uid) async {
    ...
    if (notification.type == 'official_announcement' ||
        notification.relatedAnnouncementId != null) {
      await Navigator.push(
        context,
        MaterialPageRoute(builder: (_) => const SupportAnnouncementsScreen()),
      );
      return;
    }
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(content: Text('No additional action is available.')),
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          TextButton(
            onPressed: () => _service.markAllRead(uid),
            ...
            child: ListView.separated(
              ...
              itemBuilder: (context, index) {
                ...
                return ListTile(
                  ...
                  subtitle: Text(
                    item.body.isEmpty ? 'Open for details' : item.body,
                    maxLines: 2,
                    overflow: TextOverflow.ellipsis,
                  ),
                  trailing: Text(
