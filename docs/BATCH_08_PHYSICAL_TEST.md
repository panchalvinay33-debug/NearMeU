...
- Without the app, the shared URL shows a limited web preview and a Play Store route.
- A signed-in app resolves the public identifier through a trusted callable before opening the profile.
...
2. Settings → Share My Profile opens profile-sharing controls.
3. First link generation returns an HTTPS link using an opaque public identifier, not UID/email/exact location.
...
11. Reset link: old URL no longer resolves; new URL resolves.
12. Block bypass test: block either direction, then shared link must not open the blocked profile in app.
13. Unblock restores normal resolution when link remains enabled.
14. Suspended/closed/unavailable account shared link does not expose profile.
15. Web preview shows only limited public-safe profile information and no UID/email/exact location.
16. Regression smoke: Nearby, Chats, text send, media availability, Clear/Delete semantics and Premium recovery startup remain normal.
