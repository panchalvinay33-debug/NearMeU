...
- Delete for Me removes that user's recovery copy of the affected message.
- Delete for Everyone / unsend removes recovery copies for both participants.
- Restore must never resurrect content at or before the user's authoritative Clear Chat cutoff.
...
7. Delete for Me does not return for that user after restore.
